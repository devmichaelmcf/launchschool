...
j = 0.01 #If APR is 12, monthly interest is 0.01/1% compound added monthly.
...
loop do # outer loop start
  ...
  ----------------------------------------")
    ...
    apr_string = ""
    loop do
      prompt("Enter APR % rate (including the decimal point). eg 12.4 or 9.0")
      apr_string = gets.chomp
      if valid_float?(apr_string)
        break
      else
        prompt("Invalid input! Enter APR float. eg. 12% APR please input 12.0")
      end
    end
    ...
    duration_string = ""
    loop do
      prompt("Enter the repayment period in years. e.g. 12 yr 6 months as 12.5")
      duration_string = gets.chomp
      if valid_float?(duration_string)
        break
      else
        prompt("Invalid input! Enter years including decimal. e.g. 10.0 if 10")
      end
    end
  ...
  prompt("Your monthly repayment amount is £#{monthly_repayment.round(2)}.")
